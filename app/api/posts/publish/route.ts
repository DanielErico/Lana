...
export async function POST(req: NextRequest) {
  ...
  if (!imageUrls?.length) {
    await supabaseAdmin.from('posts').update({ status: 'failed' }).eq('id', postId);
    return NextResponse.json({ error: 'Failed to render slide images' }, { status: 500 });
  }
  ...
  if (!containerIds.length) {
    await supabaseAdmin.from('posts').update({ status: 'failed' }).eq('id', postId);
    return NextResponse.json({ error: 'All slide uploads failed' }, { status: 500 });
  }
  ...
  if (!carouselData.id) {
    await supabaseAdmin.from('posts').update({ status: 'failed' }).eq('id', postId);
    return NextResponse.json({ error: 'Failed to create carousel container', detail: carouselData }, { status: 500 });
  }
  ...
  if (publishData.id) {
    ...
    await supabaseAdmin.from('posts').update({ status: 'failed' }).eq('id', postId);
    return NextResponse.json({ error: 'Publish failed', detail: publishData }, { status: 500 });
  }
}
